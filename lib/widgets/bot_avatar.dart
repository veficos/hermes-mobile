...
class _BotAvatarState extends State<BotAvatar> with SingleTickerProviderStateMixin {
  ...
  bool get _hasImage =>
      _decodeImage(botAppearance(widget.name, widget.metadata).image) != null;

  void _syncTicker() {
    final shouldAnimate = widget.working && !_hasImage;
  ...
  Widget build(BuildContext context) {
    ...
    final image = _decodeImage(appearance.image);
    return Semantics(
      ...
      child: SizedBox(
        ...
        child: ClipOval(
          child: image != null
              ? Image.memory(image, width: widget.size, height: widget.size, fit: BoxFit.cover)
              : CustomPaint(
...
class _BotFacePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final body = shape == 'cloud'
        ...
        : ringToPath(
            sampleFaceRing(shape)
                .map((p) => projectFacePoint(p.dx, p.dy, pose.turn, pose.tilt, pose.roll))
                .toList(),
          );
    ...
    if (!pose.blink) {
      ...
      canvas.drawOval(Rect.fromCenter(center: Offset(eyeL, eyeY), width: 4.4, height: eyeRy * 2), eyePaint);
      canvas.drawOval(Rect.fromCenter(center: Offset(eyeR, eyeY), width: 4.4, height: eyeRy * 2), eyePaint);
      ...
      canvas.drawLine(Offset(eyeL - 2.6, eyeY), Offset(eyeL + 2.6, eyeY), shutPaint);
      canvas.drawLine(Offset(eyeR - 2.6, eyeY), Offset(eyeR + 2.6, eyeY), shutPaint);
    }

    if (working) {
      canvas.drawCircle(const Offset(16.4, 41.2), 1.15, Paint()..color = color.withValues(alpha: pose.d0));
      canvas.drawCircle(const Offset(20, 41.2), 1.15, Paint()..color = color.withValues(alpha: pose.d1));
      canvas.drawCircle(const Offset(23.6, 41.2), 1.15, Paint()..color = color.withValues(alpha: pose.d2));
    }
